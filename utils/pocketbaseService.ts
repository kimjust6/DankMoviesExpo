...
const EXPO_PUBLIC_POCKETBASE_URL =
  process.env?.EXPO_PUBLIC_POCKETBASE_URL ?? "https://movies.jkim.win";

// Initialize PocketBase client
const pb: TypedPocketBase = new PocketBase(
  EXPO_PUBLIC_POCKETBASE_URL,
) as TypedPocketBase;
...
export async function getAllUsers() {
  return await getFullList<UsersResponse>(Collections.Users, {
    sort: "-created",
  });
}

export async function searchUsers(query: string) {
  return await pb.collection(Collections.Users).getList<UsersResponse>(1, 50, {
...
export async function addWatchHistoryItem(data: Partial<WatchHistoryResponse>) {
